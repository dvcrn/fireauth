defmodule Fireauth.FirebaseTest do
  ...
  test "verify_id_token/2 rejects non-JWT tokens" do
    assert {:error, :invalid_token_format} = Firebase.verify_id_token("nope")
    assert {:error, :invalid_token_format} = Firebase.verify_id_token("a.b")
    assert {:error, :invalid_token_format} = Firebase.verify_id_token("a.b.c.d")
  end
