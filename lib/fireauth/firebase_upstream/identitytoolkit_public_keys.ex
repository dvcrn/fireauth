defmodule Fireauth.FirebaseUpstream.IdentityToolkitPublicKeys do
  @moduledoc """
  Caches the public keys used to verify Firebase session cookies.

  Keys are fetched from the Identity Toolkit endpoint and cached based on the
  ...
  @keys_url "https://www.googleapis.com/identitytoolkit/v3/relyingparty/publicKeys"
  ...
  def refresh_keys do
    ensure_started!()
    now_s = now_s()
    Logger.debug("fireauth: refreshing Identity Toolkit public keys")
  ...
  # Avoid network requests on app boot; fetch lazily on first use.
  defp init_state(_opts), do: %{keys: %{}, expires_at_s: 0}

  defp fetch_keys do
    Logger.debug("fireauth: downloading public keys from #{@keys_url}")

    case Req.get(@keys_url, decode_body: false, redirect: true) do
  ...
  defp ensure_started! do
    case Process.whereis(__MODULE__) do
      nil ->
        raise "Fireauth.FirebaseUpstream.IdentityToolkitPublicKeys is not started (start the :fireauth application)"
